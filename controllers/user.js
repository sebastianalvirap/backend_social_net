...
//Método para subi AVATAR
export const uploadavatar = async (req,res) => {
  try {
    //Verificar su se ha subido un archivo
    if(!req.file){
      return res.status(400).send({
        status: "error",
    ...
    //Obtener la url del archivo subido en Loudinary
    const avatarUrl = req.file.path;

    //Guardar la imagen en la BDD
    ...
    console.log("Error al subir el archivo delavatar", error)
    return res.status(500).send({
      status: "error",
      message: "Error al subir el archivo del avatar"
    });
...
// Método para mstrr el AVATAR (imagen de perfil)
export const avatar = async (req,res) =>{
  try {
    ...
    console.log("Error al mostrar archivo delavatar", error)
    return res.status(500).send({
      status: "error",
